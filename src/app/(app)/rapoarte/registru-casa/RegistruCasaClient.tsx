...
// Calculează soldul curent (rulant) pentru fiecare operațiune
...
async function generateAndDownloadPdf(
  ...
  const docDefinition: any = {
    ...
    content: [
      ...
      { text: "REGISTRU DE CASĂ", style: "title", alignment: "center", margin: [0, 14, 0, 14] },
      {
        table: {
          headerRows: 1,
  ...
  const fileName = `registru-casa-${dataStart}-${dataEnd}.pdf`;
  const pm = pdfMake.default ?? pdfMake;
  pm.createPdf(docDefinition).download(fileName);
}

export default function RegistruCasaClient({
  defaultStart,
  defaultEnd,
}: {
  defaultStart: string;
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`/api/rapoarte/registru-casa?${params}`);
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      await generateAndDownloadPdf(freshAsoc, soldInitial, ops, dataStart, dataEnd);
    } catch (e: any) {
    ...
    <>
      ...
      <div className="page-shell">
        <div className="page-header">
          <div>
            <h1 className="page-title">Registru casă</h1>
      ...
      {ops.length === 0 && !loading ? (
        <div className="empty-state">
          ...
          <div className="empty-state__title">Nicio operațiune prin casă în perioada selectată</div>
        </div>
      ) : (
      ...
      <div id="print-zone">
        ...
        <div style={{ textAlign: "center", fontSize: "15pt", fontWeight: "bold", letterSpacing: "1pt", textTransform: "uppercase", margin: "10pt 0 12pt" }}>
          Registru de casă
        </div>
